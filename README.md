...
This README.md introduces the Image Plotter project, a ME 405 project. It includes the overviews of the implemented hardware and software as well as aa discussion of the results and what we have learned.
...
We drew another simple shape of a circle. this drawing was also pretty good, but we did not have smooth curves. The scaling of the circle was obviously skewed. See figures 6 and 7. The image looked good and we were extremely happy with these results.
...
For anyone who would like to build on our device, we have several recommendations could help. First, improve the way the solenoid is attached to the pen. Currently it is only attached by a quick solder. This allows the pen to rotate while drawing which ruins the accuracy of our lines. If the connection were rigid the pen would be able to draw more accurately. Additional parameter tuning is required to optimize the system the Ki and Kd are currently zero and the motor period and Kp could need more tuning. This would allow the drawing to get to each point more accurately and may even allow a decreased tolerance. Decreasing the number of points in a contour would greatly improve the speed at which the machine prints.  
